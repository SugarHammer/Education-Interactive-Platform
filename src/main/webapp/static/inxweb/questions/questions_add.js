...
function addQuestions(){
	...
	if(title.trim()==""){
		$("input[name='questions.title']").parent().next().html('<img width="16" height="16" alt="错误"  ');
		...
		$("input[name='questions.title']").parent().next().html('<img width="16" height="16" alt="正确" ');
	}

	var questionsContent=$("textarea[name='questions.content']").val();
	if(questionsContent.trim()==""){
		$("textarea[name='questions.content']").next().html('<img width="16" height="16" alt="错误"  ');
		dialog('提示信息',"问答内容不能为空",1);
		return;
	}else{
		$("textarea[name='questions.content']").next().html('<img width="16" height="16" alt="正确"  ');
	}

	var questionsTag=$("#questionsTag").val().trim();
	if(questionsTag==""){
		$("#tags-content").next().html('<img width="16" height="16" alt="错误"  ');
		dialog('提示信息',"请至少选择一个标签",1);
		return;
	}else{
		$("#tags-content").next().html('<img width="16" height="16" alt="正确"  ');
	}

	var randomCode=$("input[name='randomCode']").val();
	if(randomCode.trim()==""){
		$("input[name='randomCode']").parent().next().next().html('<img width="16" height="16" alt="错误"  ');
		dialog('提示信息',"请输入验证码",1);
		return;
	}else{
		$("input[name='randomCode']").parent().next().next().html('<img width="16" height="16" alt="正确"  ');
	}
...
function checkTitleLength(obj){
	...
	if(title.length<16){
		$(obj).parent().next().html('<img width="16" height="16" alt="错误"  ');
	}else{
		$(obj).parent().next().html('<img width="16" height="16" alt="正确"  ');
	}
}
...
function checkQuestionContent(obj){
	var title=$(obj).val().trim();
	if(title==""){
		$(obj).next().html('<img width="16" height="16" alt="错误"' );
	}else{
		$(obj).next().html('<img width="16" height="16" alt="正确"  ');
	}
}